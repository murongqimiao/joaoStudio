 ...
 * 
 */
export const checkMapRemora = function(mapRule, v, fail, success, config) {
    let variationX = config && config.variationX || 0
    let variationY = config && config.variationY || 0
    ...
    let checkResult = mapRule.map(mapRuleItem => {
        ...
        return checkPointInMap(_scaledPoints, { x: v.position.x + distanceMapX + variationX, y: v.position.y + distanceMapY + variationY}, mapRuleItem.type)
    })
