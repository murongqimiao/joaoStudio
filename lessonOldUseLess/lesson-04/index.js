            ...
            if (v.oldPosition && v.oldPosition.length) { // 移动过的元素才需要进行碰撞检测
                this.allRenderList.forEach((item, itemIndex) => {
                    ...
                    if (v.currentId !== item.currentId && v.curRender.lastFrame && item.curRender.lastFrame && collisionDetection(v, item)) {
                        ...
                        console.log(v)
                        if (v.state.isSolid && item.state.isSolid) {
                            v.crash = true
                        }
                    }
                })
...
class Role {
    ...
    addPosition(params) {
        ...
        let oldPositionDeepCopy = JSON.parse(JSON.stringify(this.position))
        if (!this.position.x) {
            // 首次添加, 不需要记录oldposition
        } else {
            if (!this.oldPosition || !this.oldPosition.length) {
                this.oldPosition = [].concat(oldPositionDeepCopy)
            } else {
                this.oldPosition.push(oldPositionDeepCopy)
            }
        }
...
footManNew.addPosition({ x: 300, y: 300, z: 0, yRegression: 5 }).addAction('action', walking)
goldCoinInMapNew.addPosition({ x: 100, y: 100, z: 0, yRegression: 5 })
Monster01New.addPosition({ x: 0, y: 200, z: 0, yRegression: 5 })
